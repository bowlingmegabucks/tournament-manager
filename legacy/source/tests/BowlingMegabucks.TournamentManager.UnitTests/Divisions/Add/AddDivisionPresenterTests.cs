...
{
    private Mock<TournamentManager.Divisions.Add.IView> _view;
    private Mock<TournamentManager.Divisions.Retrieve.IAdapter> _retrieveAdapter;
    private Mock<TournamentManager.Divisions.Add.IAdapter> _addAdapter;

    private TournamentManager.Divisions.Add.Presenter _presenter;

    [SetUp]
    public void SetUp()
    {
        _view = new Mock<TournamentManager.Divisions.Add.IView>();
        _retrieveAdapter = new Mock<TournamentManager.Divisions.Retrieve.IAdapter>();
        _addAdapter = new Mock<TournamentManager.Divisions.Add.IAdapter>();

        _presenter = new TournamentManager.Divisions.Add.Presenter(_view.Object, _retrieveAdapter.Object, _addAdapter.Object);
    }

    [Test]
    public async Task GetNextDivisionNumberAsync_RetrieveDivisionsAdapterExecute_CalledCorrectly()
    {
        var division = new Mock<TournamentManager.Divisions.IViewModel>();
    ...
    public async Task GetNextDivisionNumber_RetrieveDivisionAdapterHasNoErrors_NextDivisionNumberSet(short divisionCount, short expected)
    {
        ...
        var divisions = Enumerable.Repeat(new Mock<TournamentManager.Divisions.IViewModel>(), divisionCount).Select(mock => mock.Object).ToList();
