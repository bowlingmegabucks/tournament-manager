...
{
    private Mock<TournamentManager.Tournaments.Add.IView> _view;
    private Mock<TournamentManager.Tournaments.Add.IAdapter> _adapter;

    private TournamentManager.Tournaments.Add.Presenter _presenter;

    [SetUp]
    public void SetUp()
    {
        _view = new Mock<TournamentManager.Tournaments.Add.IView>();
        _adapter = new Mock<TournamentManager.Tournaments.Add.IAdapter>();

        _presenter = new TournamentManager.Tournaments.Add.Presenter(_view.Object, _adapter.Object);
    }
    ...
    public async Task ExecuteAsync_ViewIsValidFalse_InvalidViewFlow()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            _adapter.Verify(adapter => adapter.ExecuteAsync(It.IsAny<TournamentManager.Tournaments.IViewModel>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    public async Task ExecuteAsync_ViewIsValidTrue_AdapterExecute_CalledCorrectly()
    {
        ...
        _adapter.Setup(adapter => adapter.ExecuteAsync(It.IsAny<TournamentManager.Tournaments.IViewModel>(), It.IsAny<CancellationToken>())).ReturnsAsync(TournamentId.Empty);
    ...
    public async Task ExecuteAsync_ViewIsValidTrue_AdapterHasErrors_AdapterErrorFlow()
    {
        ...
        var errors = Enumerable.Repeat(new TournamentManager.Models.ErrorDetail("message"), 3);
    ...
    public async Task ExecuteAsync_ViewIsValidTrue_AdapterHasNoErrors_SuccessFlow()
    {
        ...
        _adapter.Setup(adapter => adapter.ExecuteAsync(It.IsAny<TournamentManager.Tournaments.IViewModel>(), It.IsAny<CancellationToken>())).ReturnsAsync(id);
